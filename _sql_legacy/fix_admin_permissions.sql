-- Script para solucionar problemas de permisos y generar subastas programadas

-- 1. Verificar usuario actual
SELECT 'Usuario actual:' as step;
SELECT auth.uid() as user_id, auth.email() as email;

-- 2. Asegurar que el usuario tiene rol admin
SELECT 'Asignando rol admin...' as step;
INSERT INTO public.user_roles (user_id, role)
VALUES (auth.uid(), 'admin')
ON CONFLICT (user_id) DO UPDATE SET role = 'admin';

-- 3. Verificar que ahora tiene el rol
SELECT 'Verificando rol asignado:' as step;
SELECT role FROM public.user_roles WHERE user_id = auth.uid();

-- 4. Crear función simplificada para generar subastas sin verificación de admin (temporal)
...
BEGIN
  -- REMOVIDO: Verificación de admin para facilitar pruebas
...
-- 6. Probar la función simplificada
